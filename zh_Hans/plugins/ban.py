...
banData = {}
configDir = "plugins/py/ban/"

def init():
    global banData
    if not os.path.exists(configDir):
        os.makedirs(configDir)
    if not os.path.exists(configDir + 'ban.json'):
        with open(configDir + 'ban.json', 'w') as file:
            file.write(json.dumps({}, sort_keys=True, indent=4, separators=(',', ': ')))
    with open(configDir + 'ban.json', 'r') as file:
        banData = json.load(file)
    ...
    if cmd[0] == "/ban" or cmd[0] == "/unban":
        ...
        if len(cmd) == 1:
            if cmd[0] == "/unban":
                ...
                admin.sendTextPacket("用法: /ban <玩家名> <时间: 秒数或形如2022.04.01-11.45.14的具体时间, 不填为永久> <原因:选填>")
            return False
...
def onConsoleCmd(data):
    ...
    if cmd[0] == "ban" or cmd[0] == "unban":
        if len(cmd) == 1:
            ...
            if cmd[0] == "unban":
                logout("用法: ban <玩家名> <时间: 秒数或形如2022.04.01-11.45.14的具体时间, 不填为永久> <原因:选填>", level = "ERROR")
            return False
        for player in mc.getPlayerList():
            if player.name == cmd[1]:
                playerXuid = player.xuid
...
def addBanPlayer(playerName, xuid = "unknown", IP = "unknown", time = "forever", reason = "unknown"):
    ...
    if playerName.lower() in banData:
        banData[playerName.lower()]["info"].append(f'{getTime()} {time} {IP} {reason}')
    else:
        banData.update({playerName.lower(): {'info': [f'{getTime()} {time} {IP} {reason}'], 'name': playerName, 'xuid': xuid}})

def saveBanList():
    with open(configDir + 'ban.json', 'w') as file:
        file.write(json.dumps(banData, sort_keys=True, indent=4, separators=(',', ': ')))
...
def toSecond(banTime):
    ...
    seconds = intBanTime[0] * 365 * 24 * 60 * 60 + intBanTime[1] * 30 * 24 * 60 * 60 + intBanTime[2] * 24 * 60 * 60 + intBanTime[3] * 60 * 60 + intBanTime[4] * 60 + intBanTime[5]
    return seconds
...
mc.setCommandDescription('ban', '/ban <玩家名> <时间: 秒数或形如2022.04.01-11.45.14的具体时间, 不填为永久> <原因:选填>')
